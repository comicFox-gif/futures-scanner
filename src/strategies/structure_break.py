        ...
        if price > sh and float(prev["close"]) <= sh:
            ...
            if detect_bull_trap(entry_df, f"ema_{self.ema_slow}"):
                if bull_trap_short_confirmed(entry_df):
                    logger.debug(f"[BOS] {symbol} bull trap → fading with SHORT")
                    return {
                        ...
                        "reason": f"Bull Trap ↓ Fade | BOS pump overextended | RSI={rsi:.0f}",
                    }
                logger.debug(f"[BOS] {symbol} LONG blocked — bull trap (no wick confirmation)")
                return None
